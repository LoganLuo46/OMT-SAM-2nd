...
def build_loaders(root_dir, batch_size=1, num_workers=0):
    ...
    try:
        full_ds = OrganSliceDataset(root_dir, organ_id=target_oid)
        train_ds = Subset(full_ds, indices=[0])
        val_ds = Subset(full_ds, indices=[0])

        loaders[target_oid] = {
            "train": DataLoader(train_ds, batch_size=batch_size, shuffle=False, num_workers=num_workers),
...
def main():
    ...
    parser.add_argument("--use_clip", type=bool, default=True,
                        help="Whether to use CLIP model for text and image prompt fusion")
